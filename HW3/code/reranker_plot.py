...
class LossTrackingCrossEncoder(CrossEncoder):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.current_loss = None  # Store current batch loss
        
    def fit(self, train_dataloader, evaluator, epochs, evaluation_steps, warmup_steps,
            output_path, save_best_model, optimizer_params, show_progress_bar, 
            gradient_accumulation_steps=1, callback=None, **kwargs):
        """
        Override fit to intercept and track training loss
        """
        import torch
        from torch.utils.data import DataLoader
        ...
        # Use self.device instead of self._target_device
        device = self.device
        ...
        for epoch in trange(epochs, desc="Epoch", disable=not show_progress_bar):
            ...
            for step, batch in enumerate(train_dataloader):
                # DataLoader already uses smart_batching_collate, so batch is (features, labels)
                ...
                # Evaluation (only if we have completed at least one step AND it's time to evaluate)
                if evaluation_steps > 0 and global_step > 0 and global_step % evaluation_steps == 0:
                    self.model.eval()
                    ...
                    if self.batch_losses:
                        ...
                        logging.info(f"\n{'='*80}")
                        logging.info(f"[Epoch {epoch+1}, Step {global_step}] Average Training Loss: {avg_loss:.4f}")
                        self.batch_losses = []
                    
                    # Compute IR metrics
                    if self.dev_queries:
                        logging.info(f"[Epoch {epoch+1}, Step {global_step}] Computing IR metrics...")
                        ir_metrics = self.compute_ir_metrics()
                        self.all_step_metrics.append(ir_metrics)
                        logging.info(f"[Epoch {epoch+1}, Step {global_step}] MRR@10: {ir_metrics.get('mrr@10', 0):.4f}, "
                                   f"NDCG@10: {ir_metrics.get('ndcg@10', 0):.4f}, "
                                   f"MAP@10: {ir_metrics.get('map@10', 0):.4f}, "
                                   f"Accuracy@10: {ir_metrics.get('accuracy@10', 0):.4f}")
                        logging.info(f"{'='*80}\n")
                    
                    # Run evaluator (optional, can be disabled to save time)
                    # if evaluator is not None:
                    #     score = evaluator(self, output_path=output_path, epoch=epoch, steps=global_step)
                    #     if isinstance(score, dict):
                    #         score_value = score.get('ap', score.get('accuracy', 0.0))
                    #     else:
                    #         score_value = score
                    #     
                    ...
                    if callback is not None:
                        callback(self.best_score, epoch, global_step)
        ...
        if self.batch_losses:
            ...
            self.step_numbers.append(global_step)
            logging.info(f"\n{'='*80}")
            logging.info(f"[Final, Step {global_step}] Average Training Loss: {avg_loss:.4f}")
            self.batch_losses = []
        
        if self.dev_queries:
            logging.info(f"[Final, Step {global_step}] Computing final IR metrics...")
            ir_metrics = self.compute_ir_metrics()
            self.all_step_metrics.append(ir_metrics)
            logging.info(f"[Final, Step {global_step}] MRR@10: {ir_metrics.get('mrr@10', 0):.4f}, "
                       f"NDCG@10: {ir_metrics.get('ndcg@10', 0):.4f}, "
                       f"MAP@10: {ir_metrics.get('map@10', 0):.4f}, "
                       f"Accuracy@10: {ir_metrics.get('accuracy@10', 0):.4f}")
            logging.info(f"{'='*80}\n")
    ...
    def compute_ir_metrics(self):
        ...
        for query, passages in self.dev_queries.items():
            ...
            # Score all passages for this query (disable progress bar)
            pairs = [(query, p[0]) for p in passages]
            scores = self.predict(pairs, show_progress_bar=False)
    ...
    # Remove the old fit override since we have a new one above
...
def main():
    ...
    # Use smart_batching_collate as collate function for DataLoader
    ...
    logging.info(f"Total training steps: {len(train_dataloader) * args.epochs}")
    logging.info(f"Evaluation every {args.eval_steps} steps")
